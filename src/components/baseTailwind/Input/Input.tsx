...
const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const inputStyles = {
      ...
      dark:{
        common: [tw`text-base-light-10 placeholder:text-base-light-10`,
          tw`caret-blue-light-550 w-full bg-blue-dark-200 rounded-l`,
          css` & .${inputBaseClasses.input} {
    ...
    if (height){ heightClass = `h-[${height}px]`; }

    const divStyles = {
      default: [
        tw`border outline-none h-[53px]`,
        css`border-radius: 2px!important`,
        height ? css`height: ${height}px!important` : ""
      ],
      dark:{
       common: [
          tw`bg-blue-dark-200`,
         !noBorder ? tw`border-gray-dark-100` : css`border-style: none!important`,
    ...
    const themes = {
      dark: {
        ...
        inputRightCss: [css` &, & * {color: rgba(255, 255, 255, 0.5);}`],
        inputLabelCss: [
          ...
          css`
                  ...
                  &.MuiInputLabel-shrink {
                    ...
                    ${tw`ml-0 bg-white pl-1`}
                     background: transparent;                    
                     color:  rgba(255, 255, 255, 0.5); //цвет лэйбла при заполненном инпуте
...
//191D31
    return (
      <FormControl fullWidth css={[readOnly && tw`[*]:pointer-events-none!`]}>
        <div css={[tw`flex flex-col rounded-lg`]}>
         ...
         {  left && (
            ...
            <InputBase
              ...
              id={randomId}
              css={[...themes[theme].inputBaseCss, !label && css` 
               &.${inputBaseClasses.root} {
                    align-items: center;
                  } `]}
              onFocus={() => {
            ...
            {right && (
              <div
                ...
                css={[css`align-self: center; padding-right: 16px;`, ...themes[theme].inputRightCss ]}
              >
                {right}
              </div>
            )}
        ...
        {bottom && (
          <div css={[tw`flex mt-2 text-xs`, css`color: rgba(255, 255, 255, 0.7)`]}>{bottom}</div>
        )}
