...
function checkFiles(
  ...
  if (maxFiles && files.length > maxFiles) {
    throw new Error(`You can only upload ${maxFiles} ${maxFiles === 1 ? "file" : "files"}`);
  }
    ...
    <div>
      {files?.length === 0 && <div
        ...
        css={[
          ...
          css` border-color: rgba(255, 255, 255, 0.5);`,
          tw`h-48`,
          isDragActive && css`border-color: rgba(255, 255, 255, 0.9); `,
      ...
      >
        {loading ? <div style={{color: "white"}}>{loading && "loading..."}</div> :
          <div className={styles.innerContent}>
            ...
            <div className={styles.extensionsRow}>
              .odt, .doc, docx, .pdf, .jpg, jpeg, .png, .ppt, .odp, .pptx <br/> (max 3MB)
            </div>
          </div>
      ...
      <div css={tw`min-h-full justify-center flex flex-col gap-2`}>
        {(error  || (variant === "error" && message && message?.length > 0)) && <div css={tw`text-red-light-450`}>
      ...
      {!error && files && files.length > 0 && (
        <div style={{marginTop: "10px"}}>
          {files.map((file, key) => {
            return (
              <div key={key} className={`${styles.FileWrap} ${styles['FileWrap_' + theme]}`}>
                <div className={styles.FileName}>{file?.file?.name}</div>
                <div
                  css={tw`hover:text-red-light-450 text-base-light-10 cursor-pointer w-5`}
                  onClick={() => {
                    const leftFiles = files.filter((f) => f?.file?.name !== file?.file?.name)?.[0]?.file;
